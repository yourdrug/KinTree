...
export default function Dashboard() {
  ...
  const loadTrees = async () => {
    try {
      ...
      toast({ variant: "destructive", title: "Ошибка загрузки", description: "Не удалось загрузить список деревьев." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const createTree = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Ошибка создания", description: "Не удалось создать дерево." });
    }
  };
      ...
      <main className="max-w-6xl mx-auto px-6 md:px-10 py-10">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <Input placeholder="Поиск деревьев..." value={search}
        ...
        {creating && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <Input autoFocus placeholder="Название дерева" value={newTreeName}
...
function SkeletonGrid() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {[1, 2, 3].map((i) => (
        <div key={i} className="h-40 rounded-2xl animate-pulse" style={{ background: "hsl(35,25%,93%)" }} />
      ))}
    </div>
  );
}
