...
export default function ResetPassword() {
  ...
  if (!token) return <ErrorScreen message="Ссылка недействительна. Запросите новую." />;
  ...
  if (status === "success") {
    return (
      <CenteredLayout>
        ...
        <h1 className="font-serif text-2xl font-semibold text-foreground mb-3">Пароль изменён</h1>
        <p className="text-sm text-muted-foreground mb-8">
          Новый пароль установлен. Все активные сессии завершены.
        </p>
        <Button onClick={() => nav.toLogin()} className="w-full h-12 rounded-xl text-sm font-semibold gap-2"
  ...
  return (
    <CenteredLayout>
      <h1 className="font-serif text-3xl font-semibold text-foreground mb-2">Новый пароль</h1>
      <p className="text-sm text-muted-foreground mb-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <Input id="password" type={showPass ? "text" : "password"} value={password}
              ...
              autoComplete="new-password" className="pl-10 pr-12 h-12 rounded-xl text-sm"
              style={{ background: "white", border: "1.5px solid hsl(35,20%,88%)" }} />
            <button type="button" tabIndex={-1} onClick={() => setShowPass((v) => !v)}
              className="absolute right-3.5 top-1/2 -translate-y-1/2 w-7 h-7 flex items-center justify-center rounded-lg hover:bg-muted">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="confirm" className="text-xs font-medium text-muted-foreground">Повторите пароль</Label>
          <div className="relative">
            <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input id="confirm" type={showConfirm ? "text" : "password"} value={confirm}
              onChange={(e) => setConfirm(e.target.value)} placeholder="Введите пароль ещё раз"
              autoComplete="new-password" className="pl-10 pr-12 h-12 rounded-xl text-sm"
              style={{ background: "white", border: "1.5px solid hsl(35,20%,88%)" }} />
