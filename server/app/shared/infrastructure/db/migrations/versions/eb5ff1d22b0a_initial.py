...
Revision ID: eb5ff1d22b0a
Revises: dded7f72bf75
Create Date: 2026-05-05 20:51:41.575229
...
def upgrade() -> None:
    ...
    sa.Column('account_id', sa.String(), nullable=False, comment='Владелец токена'),
    sa.Column('session_id', sa.String(length=64), nullable=False, comment='Уникальный ID сессии, вшивается в access и refresh токены'),
    sa.Column('token_hash', sa.String(length=64), nullable=False, comment='SHA-256 hex от raw refresh token — raw никогда не хранится'),
    sa.Column('expires_at', sa.DateTime(timezone=True), nullable=False, comment='Время истечения токена'),
    sa.Column('revoked', sa.Boolean(), nullable=False, comment='True = токен отозван (logout или rotation)'),
    sa.Column('user_agent', sa.String(length=512), nullable=True, comment='User-Agent браузера/приложения при создании сессии'),
    sa.Column('ip_address', sa.String(length=45), nullable=True, comment='IP при создании сессии'),
    sa.Column('id', sa.String(), nullable=False, comment='Entity identifier (UUID hex)'),
    sa.Column('creation_date', sa.DateTime(timezone=True), nullable=False, comment='Timestamp with timezone'),
    sa.ForeignKeyConstraint(['account_id'], ['Account.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('session_id'),
    sa.UniqueConstraint('session_id', name='uq_refresh_token_session')
    )
    op.create_index('idx_rt_account_id', 'refresh_tokens', ['account_id'], unique=False)
    op.create_index('idx_rt_session_id', 'refresh_tokens', ['session_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index('idx_rt_session_id', table_name='refresh_tokens')
    op.drop_index('idx_rt_account_id', table_name='refresh_tokens')
